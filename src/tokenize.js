...
import {
  AST,
  BinaryOperatorPrecedence as Precedence,
} from '@robinblomberg/sqlite-ast';
...
const __ChildExpression = (node, parent) => {
  /** @type {Token[]} */
  const t = [];
  const childHasLowerPrecedence =
    node.type === '_BinaryExpression' &&
...
const __PathLike = (node) => {
  return node.type === '_Path' ? _Path(node) : _Identifier(node);
};
...
/**
 * @param {AST._ArrayExpression} node
 * @return {Token[]}
 */
export const _ArrayExpression = (node) => {
  /** @type {Token[]} */
  const t = [];
...
export const _ColumnConstraintClause = (node) => {
  switch (node.type) {
    ...
    default:
      throw new TypeError(
        `Unexpected column constraint clause type: ${
          /** @type {any} */ (node).type
        }`,
      );
  }
...
export const _Stmt = (node) => {
  ...
  switch (node.type) {
    ...
    default:
      throw new TypeError(
        `Unexpected node type: ${/** @type {any} */ (node).type}`,
      );
  }
...
export const _StringLiteral = (node) => {
  return [Tokens.String(`'${node.value.replaceAll(/(['\\])/g, '\\$1')}'`)];
};
...
export const ColumnNameList = (node) => {
  ...
  t.push(Tokens.Punctuator('('));
  t.push(
    ...__List(node.columnNames, (columnName) => [
      Tokens.Identifier(columnName.name),
    ]),
  );
  t.push(Tokens.Punctuator(')'));
...
export const CreateTriggerStmt = (node) => {
  ...
  for (const stmt of node.begin) {
    switch (stmt.type) {
      ...
      default:
        throw new TypeError(
          `Unexpected node type: ${/** @type {any} */ (stmt).type}`,
        );
    }
...
export const FactoredSelectStmt = (node) => {
  ...
  for (let i = 1; i < node.selectors.length; i++) {
    t.push(
      ..._SelectCompound(
        /** @type {AST._SelectCompound} */ (node.selectors[i]),
      ),
    );
  }
...
export const JoinClause = (node) => {
  ...
  for (let i = 1; i < node.joinees.length; i++) {
    t.push(
      ..._JoinCompound(/** @type {AST._JoinCompound} */ (node.joinees[i])),
    );
  }
...
export const SelectStmt = (node) => {
  ...
  for (let i = 1; i < node.select.length; i++) {
    t.push(
      ..._SelectCompound(/** @type {AST._SelectCompound} */ (node.select[i])),
    );
  }
