...
/**
 * @param {AST.Expr[]} nodes
 * @param {(node: AST.Expr) => Token[]} tokenizeNode
 * @return {Token[]}
 */
...
export const tokenize_AsClause = (node) => {
  ...
  throw new Error('Compiler not implemented for node type "_AsClause".');
...
export const tokenize_BindParameter = (node) => {
  return [Tokens.Identifier(node.bindParameter)];
};
...
export const tokenize_CallExpression = (node) => {
  ...
  tokens.push(Tokens.Identifier(node.functionName));
  tokens.push(Tokens.Punctuator('('));
...
export const tokenize_CollateClause = (node) => {
  ...
  tokens.push(Tokens.Identifier(node.collationName));
...
export const tokenize_ColumnPath = (node) => {
  ...
  if (node.tablePath !== null) {
    tokens.push(...tokenize_Path(node.tablePath));
    tokens.push(Tokens.Punctuator('.'));
  }

  tokens.push(Tokens.Identifier(node.columnName));
...
export const tokenize_Path = (node) => {
  ...
  if (node.object !== null) {
    tokens.push(Tokens.Identifier(node.object));
    tokens.push(Tokens.Punctuator('.'));
  }

  tokens.push(Tokens.Identifier(node.property));
...
export const tokenize_RenameClause = (node) => {
  ...
  if (node.from !== null) {
    tokens.push(Tokens.Identifier(node.from));
  }

  tokens.push(Tokens.Keyword('TO'));
  tokens.push(Tokens.Identifier(node.to));
...
export const tokenizeAggregateFunctionInvocation = (node) => {
  ...
  tokens.push(Tokens.Identifier(node.aggregateFunc));
  tokens.push(Tokens.Punctuator('('));
...
export const tokenizeAttachStmt = (node) => {
  ...
  tokens.push(Tokens.Identifier(node.schemaName));
  tokens.push(Tokens.Punctuator(';'));
...
export const tokenizeColumnConstraint = (node) => {
  ...
  if (node.name !== null) {
    ...
    tokens.push(Tokens.Identifier(node.name));
  }
...
export const tokenizeColumnDef = (node) => {
  ...
  if (node.typeName !== null) {
    tokens.push(Tokens.Keyword(node.typeName));
  }
...
export const tokenizeForeignKeyClause = (node) => {
  ...
  throw new Error('Compiler not implemented for node type "ForeignKeyClause".');
...
export const tokenizeTypeName = (node) => {
  ...
  throw new Error('Compiler not implemented for node type "TypeName".');
